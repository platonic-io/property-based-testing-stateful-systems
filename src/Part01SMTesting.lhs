...
Opon a failure we proceed depth-first through the tree, initially aggressively
...
For example, the generate sequence `< i_0, i_1, i_2, i_3 >` fails, so first we
check that `< i_0, i_1 >` fails, lets say it doesn't so then we are done with
...
i_3 >` is the counter example we present.
...
We can think of coverage as making sure that certains paths appear in our tree
