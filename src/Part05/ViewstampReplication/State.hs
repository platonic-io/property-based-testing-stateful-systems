...
module Part05.ViewstampReplication.State
  (module Part05.ViewstampReplication.State)
  where
...
data ClientStatus result
  = InFlight  RequestNumber OpNumber
  | Completed RequestNumber OpNumber result ViewNumber
  deriving stock (Generic, Show)

requestNumber :: ClientStatus result -> RequestNumber
requestNumber (InFlight  reqNr _opNr)                 = reqNr
requestNumber (Completed reqNr _opNr _result _viewNr) = reqNr

copNumber :: ClientStatus result -> OpNumber
copNumber (InFlight  _reqNr opNr)                 = opNr
copNumber (Completed _reqNr opNr _result _viewNr) = opNr
